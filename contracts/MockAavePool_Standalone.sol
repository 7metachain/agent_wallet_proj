 ...
 *      Optimized for Remix deployment - No external dependencies
 */
contract MockAavePool {
    ...
    function supply(
        ...
        if (token == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) {
            ...
            // Handle ERC20 token - inline call to avoid interface definition issues
    ...
    function withdraw(
        ...
        if (token == address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE)) {
            ...
            // ERC20 token - inline call to avoid interface definition issues
            (bool success, bytes memory data) = token.call(
                abi.encodeWithSignature("transfer(address,uint256)", to, withdrawAmount)
            );
